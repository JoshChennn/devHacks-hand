...
def main():
    calc_once = 0
    finger_length = [[0 for i in range(4)] for j in range(5)]
    ...
    while True:
        ...
        if results.multi_hand_landmarks is not None:
            for hand_landmarks, handedness in zip(results.multi_hand_landmarks,
                ...
                landmark_list_3d = calc_landmark_list_3d(debug_image,hand_landmarks)
                pre_proc_list_3d = pre_process_landmark_3d(landmark_list_3d)
                ...
                for i in range(5):
                    ...
                    for j in range((4 if i == 0 else 3)):
                        k = i*12+j*3
                        dist = get_z(pre_proc_list_3d[k+3:k+5],pre_proc_list_3d[0:2] if j == 0 else pre_proc_list_3d[k:k+2],finger_length[i][j])
                        fingerPos[i*4+j+1][0] = pre_proc_list_3d[k+3]
                        fingerPos[i*4+j+1][1] = pre_proc_list_3d[k+4]
                        fingerPos[i*4+j+1][2] = (fingerPos[i*4+j][2] if j != 0 else 0) + dist
                    if i == 0:
                ...
                if calc_once == 0:
                    for i in range(5):
                        for j in range(4):
                            k = i*12+j*3
                            finger_length[i][j] = math.sqrt((pre_proc_list_3d[k+3]-pre_proc_list_3d[0 if j == 0 else k])**2+(pre_proc_list_3d[k+4]-pre_proc_list_3d[1 if j == 0 else k+1])**2)
                    # print(finger_length)
                    calc_once = 1
...
def get_z(a, b, length):
    return math.sqrt(max(0,length**2-(b[0]-a[0])**2-(b[1]-a[1])**2))
